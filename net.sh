...
do
   ...
   if [ $(($i%20)) -eq 0 ];then
      echo " ifname   | in_kbits/s out_kbits/s | in_kBytes/s out_kBytes/s | in_packets/s out_packets/s |   date     time  "
      echo "--------- | ---------- ----------- | ----------- ------------ | ------------ ------------- |----------------- "
   fi
   echo $eth $bytesin $bytesout $packetsin $packetsout $(date +'%F %T') |awk '{printf("%9s | %10d %11d | %11d %12d | %12d %13d | %8s %8s\n",$1,$2/128,$3/128,$2/1024,$3/1024,$4,$5,$6,$7)}'
...
echo "--------- | ---------- ----------- | ----------- ------------ | ------------ -------------|----------------- "
echo Average $sumbytesin $sumbytesout $sumpacketsin $sumpacketsout $(date +'%F %T') |awk '{printf("%9s | %10d %11d | %11d %12d | %12d %13d | %8s %8s\n",$1,$2/128,$3/128,$2/1024,$3/1024,$4,$5,$6,$7)}'
